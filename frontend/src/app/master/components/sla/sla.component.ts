...
export class SlaComponent implements OnInit {
  frm: FormGroup;
  priority: any[] = [];
  // uniquePriorities: Set<any> = new Set();
  projects: any[] = [];
  clients: any[] = [];
  SLA: any[] = [];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.frm = this.fb.group({
      ...
      slaHours: ['', [Validators.required]],//, Validators.pattern("^[0-9]{1,2}$")
      remarks: [''],
      active:[''],
  ...
  const priority = this.frm.get('priority')?.value;

  if (projectId && priority && slaValue !== null && slaValue !== '') {
    const isDuplicate = this.checkDuplicateSLA(projectId, priority, Number(slaValue));
    if (isDuplicate) {
  ...
  getPriorityLabel(priorityId: string | number): string {
  const found = this.priorityData.find(p => p.value.toString() === priorityId.toString());
  return found ? found.label : priorityId.toString(); // fallback to ID if not found
}
  ...
  this.priorityService.getAllPriority().subscribe((response: any[]) => {
    if (response) {
      console.log('Raw Priority Data:', response);

      // Step 1: Filter only rows where active === true and status === 1
      const activeItems = response.filter(item => item.active === true && item.status === 1);

      // Step 2: Map each item to dropdown format
      ...
      console.log('Mapped priority dropdown data:', this.priorityData);
    } else {
      this.priorityData = [];
    }
  });
  ...
  onClientSelected(selectedClient: ProjectDetails) {
    ...
    if (selectedClient) {
      ...
      this.frm.get('priority')?.setErrors(null);
      this.frm.get('priority')?.reset();
...
onProjectSelected(selectedProject: ZohoProject): void {
  if (selectedProject) {
    ...
    this.frm.patchValue({
      ...
      priority: '' // clear previously selected priority
    });

    // Check if there are priorities available for the project
    const priorityControl = this.frm.get('priority');
...
onPrioritySelected(priorityId: number): void {
  ...
  // Check for duplicate SLA entry with same project + priority (regardless of SLA hours)
  const isDuplicate = this.SLA?.some(item =>
    item.projectId === projectId &&
    item.priority === priorityId.toString() &&
    item.slaId !== this.selectedId // ignore self if editing
  );

  if (isDuplicate) {
    this.frm.get('priority')?.setErrors({ duplicate: true });
  } else {
    // Remove only the duplicate error if present
    const control = this.frm.get('priority');
    const currentErrors = control?.errors;
    if (currentErrors && currentErrors['duplicate']) {
      delete currentErrors['duplicate'];
      if (Object.keys(currentErrors).length === 0) {
...
checkDuplicateSLA(projectId: string, priorityId: number, slaHours: number): boolean {
 ...
 editSLA(id: number) {
  ...
  this.slaService.getSLAById(id).subscribe((response: any) => {
    ...
    const selectedPriority = this.filteredPriorityData.find(
      p => p.value.toString() === response.priority.toString()
    );
  ...
  // Map priority ID back to string label
  const selectedPriorityObj = this.filteredPriorityData.find(
    p => p.value === values.priority
  );

  console.log('Selected priority object:', selectedPriorityObj);
  ...
  const payload = {
    ...
    priority: selectedPriorityObj?.value.toString() ?? '',  
  ...
  if (this.isEditMode) {
    this.slaService.editSLA(payload).subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'Resolution SLA Updated Successfully' });
        this.visible = false;
        this.frm.reset();
        this.ngOnInit();
      },
    ...
    this.slaService.AddSLA(payload).subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'Resolution SLA Added Successfully' });
        this.visible = false;
        this.frm.reset();
        this.ngOnInit();
      },
    ...
    this.slaService
      ...
      .subscribe((result) => {
        if (result) {
          ...
          this.messageService.add({
            severity: 'success',
            detail: 'Resolution SLA Deleted Successfully.',
          });
        }
      });
  }
